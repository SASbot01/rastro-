...
export async function GET() {
  ...
  for (const m of members) {
    ...
    rows.push([m.email, m.last_seen_at ? "activo" : "invitado", m.org_share_at ? "si" : "no", s ? String(s.score) : "", s ? String(s.passwords) : "", m.monitoring ? "si" : "no", s ? s.at.slice(0, 10) : ""]);
  }
  const csv = rows.map((r) => r.map((c) => `"${c.replace(/"/g, '""')}"`).join(",")).join("\n");
  return new NextResponse("﻿" + csv, { headers: { "content-type": "text/csv; charset=utf-8", "content-disposition": `attachment; filename="rastro-equipo-${org.id.slice(0, 8)}.csv"` } });
}
