...
(function () {
  ...
  var VARIANTS = ["vigia", "cubo", "orbe"];
  var COLORS = { green: "#4dfc5f", orange: "#ffb020", red: "#ff5f5f", idle: "#4dfc5f" };
  var W = 120, H = 172, NECK_X = 60, NECK_Y = 96;
  ...
  var CSS = [
    ...
    ".bot{position:absolute;width:" + W + "px;height:" + H + "px;cursor:grab;touch-action:none;pointer-events:auto;user-select:none;-webkit-user-select:none;filter:drop-shadow(0 10px 18px rgba(0,0,0,.55))}",
    ".bot.drag{cursor:grabbing}",
    ".bot svg{overflow:visible;display:block}",
    ".badge{position:absolute;right:-2px;top:-6px;min-width:26px;height:26px;padding:0 6px;border-radius:13px;background:#0a0a0a;border:2px solid var(--accent);color:var(--accent);font-weight:700;font-size:12px;line-height:22px;text-align:center;box-sizing:border-box}",
  ...
  function create(opts) {
    opts = opts || {};
    var labels = Object.assign({ title: "Rastro", reject: "Rechazar por mí", hide: "Ocultar aquí", more: "Ver en Rastro", privacy: "Todo se comprueba en tu navegador. No enviamos tus cookies a ningún sitio.", robots: ["Vigía", "Cubo", "Orbe"], waiting: "Mirando esta web…" }, opts.labels || {});
    var variant = VARIANTS.indexOf(opts.variant) >= 0 ? opts.variant : "vigia";
    ...
    function build() {
      ...
      svg.setAttribute("viewBox", "0 -16 " + W + " " + (H + 16)); svg.setAttribute("width", W); svg.setAttribute("height", H);
      bodyG = el("g", { class: "body" }, svg);
      body = drawBody(bodyG, accent());
      headG = el("g", { class: "headg" }, svg);
      head = drawHead(variant, headG, accent());
    ...
    function step(now) {
      ...
      head.lids.forEach(function (r) { var full = variant === "orbe" ? 64 : 34; r.setAttribute("height", String(full * Math.max(lid, dragging ? 0 : calm))); });
