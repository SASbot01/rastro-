#!/usr/bin/env python3
"""Composicion del tramo de la app: capas PNG (layers/) -> app-seg.mp4 (7,2 s, 30 fps, 1080x1920)."""
import subprocess
D = 7.2; SC = 2.4; FPS = 30
...
for i in range(1, 4):
    ...
    layers.append((f"layers/s{i}-phone.png", T0, T1, T0, 0.25, "0", f"H*{e(T0,0.75)}+14*sin(2*PI*(t-{T0:.3f})/2.4)", fo_st, fo_d))
    for k in range(1, 4):
        d0 = T0 + 0.5 + 0.2*(k-1)
        layers.append((f"layers/s{i}-chip{k}.png", T0, T1, d0, 0.3, f"-120*{e(d0,0.5)}", "0", fo_st, fo_d))
fc = []; prev = "[0:v]"
