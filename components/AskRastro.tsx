  ...
  return (
    <>
      ...
      <button
        ...
        aria-expanded={open}
        className="fixed right-4 bottom-[88px] z-40 flex h-14 w-14 items-center justify-center rounded-full border-2 border-accent bg-surface shadow-[0_8px_30px_rgba(0,0,0,0.55)] transition-transform hover:scale-105 sm:right-6 sm:bottom-6"
      >
        <img src="/brand/mascot-112.png" alt="" width={52} height={52} className="h-[52px] w-[52px] rounded-full object-cover" />
        {!open && <span aria-hidden="true" className="radar-ping absolute inset-0 rounded-full border border-accent/70" />}
      </button>
      ...
      {open && (
        <section
          ...
          className="fixed inset-x-3 bottom-[152px] z-40 flex max-h-[min(70vh,560px)] flex-col overflow-hidden rounded-card border border-line bg-surface shadow-[0_16px_50px_rgba(0,0,0,0.6)] sm:inset-x-auto sm:right-6 sm:bottom-24 sm:w-[380px]"
        >
          <header className="flex items-center gap-3 border-b border-line px-4 py-3">
            ...
            <button type="button" onClick={() => setOpen(false)} aria-label={tr("ask.close")} className="flex h-8 w-8 items-center justify-center rounded-full text-muted hover:bg-surface-2 hover:text-ink">
          ...
          <div ref={listRef} className="flex-1 overflow-y-auto px-4 py-3">
            {thread.length === 0 && (
              <ul className="grid gap-2">
                {suggested.map((q) => (
                  <li key={q}>
                    <button type="button" onClick={() => void ask(q)} className="w-full rounded-[12px] border border-line bg-surface-2 px-3.5 py-2.5 text-left text-[14px] text-ink hover:border-accent">
                      {q}
                    </button>
                  </li>
                ))}
            ...
            <ul className="grid gap-2.5">
              {thread.map((m, i) => (
                <li key={i} className={m.role === "user" ? "flex justify-end" : "flex justify-start"}>
                  <p className={"max-w-[85%] whitespace-pre-line rounded-[14px] px-3.5 py-2.5 text-[14px] leading-relaxed " + (m.role === "user" ? "bg-accent text-black" : "bg-surface-2 text-ink")}>{m.content}</p>
                </li>
              ))}
              {busy && (
                <li className="flex justify-start">
                  <p className="rounded-[14px] bg-surface-2 px-3.5 py-2.5 text-[14px] text-faint">{tr("ask.thinking")}</p>
                </li>
              )}
              {error && (
                <li>
          ...
          <form onSubmit={onSubmit} className="flex items-center gap-2 border-t border-line p-3">
            <input
              ...
              className="min-w-0 flex-1 rounded-[12px] border border-line bg-surface-2 px-3.5 py-2.5 text-[14px] text-ink placeholder:text-faint focus:border-accent focus:outline-none"
            />
            <button type="submit" disabled={busy || !text.trim()} className="rounded-[12px] bg-accent px-4 py-2.5 text-[14px] font-semibold text-black disabled:opacity-50">
              {tr("ask.send")}
            </button>
          </form>
